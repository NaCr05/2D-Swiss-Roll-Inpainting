...
This is a **PyTorch** implementation of **Denoising Diffusion Implicit Models (DDIM)**, an accelerated and deterministic extension of DDPM. This project includes simple implementations ranging from 2D toy datasets (Swiss Roll) to complex image datasets (Oxford Pets), aiming to demonstrate the Forward diffusion and DDIM reverse sampling processes.

## 📂 Project Structure

```
.
├── DDIM/                   # Core algorithm implementation
│   ├── ForwardProcess.py   # Defines the forward diffusion process (q-sample)
│   ├── ReverseProcess.py   # Defines the DDIM reverse sampling process
│   └── NoisePredictor.py   # UNet architecture and noise prediction model
├── DDIM_Image.py           # Training script for the Oxford Pets dataset
├── DDIM_Swiss_Roll.py      # Training and visualization script for the 2D Swiss Roll dataset
...
![ddim_reverse_process](Plot/ddim_reverse_process.gif)

### Image Generation Process (Cats)
*(Example Visualization)*
![ddim_cat_reverse_process](Plot/ddim_cat_reverse_process_0.gif)
...
- **DDIM Sampling**: Deterministic accelerated sampling (fewer steps than full DDPM reverse process).
